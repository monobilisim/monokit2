...
func CheckProcess(logger zerolog.Logger) {
	...
	logger.Info().Msg("Checking MariaDB processes...")
	...
	logger.Info().Msgf("Successfully retrieved MariaDB processes. %d processes found.", processCount)
	logger.Debug().Interface("processes", processes).Msg("MariaDB process details")
	...
	if lib.DBConfig.MariaDB.Alarm.Enabled {
		if processCount > processThreshold {
			alarmMessage := fmt.Sprintf("[%s] - %s - MariaDB process count has been more than the set limit %d, (%d)", pluginName, lib.GlobalConfig.Hostname, processThreshold, processCount)
			...
			/*
				if lib.GlobalConfig.Redmine.Enabled {
					...
					issueSubject := fmt.Sprintf("%s için MariaDB process limiti aşıldı", lib.GlobalConfig.Hostname)
		...
		if processCount < processThreshold {
			alarmMessage := fmt.Sprintf("[%s] - %s - MariaDB process count is back to normal (%d)", pluginName, lib.GlobalConfig.Hostname, processCount)
				...
				if lib.GlobalConfig.Redmine.Enabled {
					...
					if lastIssue.Status == down {
						issue := lib.Issue{
							ProjectIdentifier: lib.GlobalConfig.ProjectIdentifier,
							Hostname:          lib.GlobalConfig.Hostname,
							Subject:           fmt.Sprintf("%s için MariaDB process limiti aşıldı", lib.GlobalConfig.Hostname),
