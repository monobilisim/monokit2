...
func InitValkey() error {
	port := lib.DBConfig.Valkey.Credentials.Port
	if port == "" {
		port = "6379"
	}
	...
	rdb = redis.NewClient(&redis.Options{
		...
		Password:   lib.DBConfig.Valkey.Credentials.Password,
		DB:         0,
		MaxRetries: 5,
	})
	...
	if ping != "PONG" {
		return fmt.Errorf("valkey ping failed")
	}
	return nil
}

func TestValkeyReadWrite() (bool, bool) {
	if rdb == nil {
		return false, false
	}
...
func IsValkeySentinel() bool {
	...
	for _, line := range lines {
		line = strings.TrimSpace(line)
		if line == "redis_mode:sentinel" || line == "valkey_mode:sentinel" {
			return true
		}
	}
	return false
